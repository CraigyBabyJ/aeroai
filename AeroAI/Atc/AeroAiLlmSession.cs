...
public class AeroAiLlmSession : IDisposable
{
	...
	public async Task<string?> HandlePilotTransmissionAsync(string pilotTransmission, CancellationToken cancellationToken = default(CancellationToken))
	{
		...
		// Quick intercept for radio check requests to avoid LLM churn (including common typos).
		var lowerTransmission = pilotTransmission.ToLowerInvariant();
		if (lowerTransmission.Contains("radio check") || lowerTransmission.Contains("raido check") ||
		    lowerTransmission.Contains("radoi check") || lowerTransmission.Contains("radio chek"))
		{
		...
		var mismatchedToken = TryExtractCallsignToken(pilotTransmission);
		if (_context.CurrentPhase == FlightPhase.Preflight_Clearance
			&& _context.CurrentAtcUnit == AtcUnit.ClearanceDelivery
			&& !string.IsNullOrWhiteSpace(mismatchedToken)
			&& !string.IsNullOrWhiteSpace(_context.CanonicalCallsign))
		{
			var normalizedToken = NormalizeCallsign(mismatchedToken);
			var normalizedCanonical = NormalizeCallsign(_context.CanonicalCallsign);
			if (!string.Equals(normalizedToken, normalizedCanonical, StringComparison.OrdinalIgnoreCase))
			{
				Console.WriteLine($"[CALLSIGN] Mismatch: pilot='{mismatchedToken}' vs canonical='{_context.CanonicalCallsign}'");
				return "Aircraft calling, your callsign does not match the flight plan I have on file. Please state your full callsign.";
			}
		}
	...
	private string BuildDeterministicClearance(AtcContext context)
	{
		...
		string clearedTo = context.ClearanceDecision.ClearedTo ?? _context.DestinationIcao ?? "destination";
