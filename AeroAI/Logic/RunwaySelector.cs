...
public sealed class RunwaySelector : IRunwaySelector
{
	...
	/// <summary>
	/// Validate whether a specific runway is acceptable for the current weather/aircraft.
	/// Useful when preferring a SimBrief-planned runway but rejecting it if winds/length/approach are unsuitable.
	/// </summary>
	public bool IsRunwayAcceptable(NavRunwaySummary runway, WeatherInfo weather, AircraftPerformanceProfile aircraft, bool isDeparture)
	{
		var (headwind, crosswindRaw) = ComputeWindComponents(runway.TrueHeadingDegrees, weather.WindDirectionDegrees, weather.WindSpeedKnots);
		double tailwind = Math.Max(0.0, 0.0 - headwind);
		double crosswind = Math.Abs(crosswindRaw);

		if (tailwind > aircraft.MaxTailwindComponentKnots || crosswind > aircraft.MaxCrosswindComponentKnots)
		{
			return false;
		}

		int requiredLength = (int)((isDeparture ? aircraft.RequiredTakeoffDistanceFeet : aircraft.RequiredLandingDistanceFeet) * RequiredLengthMarginFactor);
		if (runway.LengthFeet < requiredLength)
		{
			return false;
		}

		if (!isDeparture && weather.IsIfr && !runway.HasIlsOrLocalizer && !runway.HasRnavApproach)
		{
			return false;
		}
