...
public class AtcPhaseEngine
{
        ...
        int cruiseFlightLevel)
    {
        ...
        // PHASE 1: DELIVERY (CLEARANCE)
        sb.AppendLine("PHASE 1 - DELIVERY (PRE-DEPARTURE)");
        sb.AppendLine("───────────────────────────────────────────────────────────────");
        sb.AppendLine($"Context: At gate, {originIcao}. SimBrief route filed, but no SID or runway preselected.");
        sb.AppendLine();
        sb.AppendLine("PILOT → DELIVERY");
        sb.AppendLine($"\"{GetClearanceDeliveryName(originIcao)} Delivery, {FormatCallsign(callsign)}, stand [GATE], IFR to {destinationIcao}, information [INFO], ready to copy.\"");
        sb.AppendLine();
    ...
    {
        var sb = new StringBuilder();
        sb.Append($"\"{FormatCallsign(callsign)}, {GetClearanceDeliveryName(originIcao)} Delivery, cleared to {destinationIcao} airport");
